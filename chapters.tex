...
The Figure \ref{encoder_decoder} presents traditional encoder-decoder arhitecture. Encoder converts an input sequence of words to a corresponding fixed size hidden vector. Each vector represents the current word and the context of the word. Every time step, it takes a vector that represents a word and pass its output to the next layer. The last layer of encoder passes its output to the first layer of the decoder. The decoder input is an output encoder vector and start token, which characterizes the beginning of the generated sentence. The generated word depends on the previous decoder state and the last generated word. Many optimizations have led to other seq2seq components, such as attention, beam search, bucketing.

\section{Attention}
A neural attention mechanism gives a neural network the ability to focus on a subset of its inputs. In equation \ref{eq:attention1} attention vector $a$ is represented by attention network $f_{\phi}$  with parameters $\phi$ and an input vector $x$. An attention glimpse $g$ in equation \ref{eq:attention2} is a element-wise multiplication of the attention vector $a$ and a feature vector $z$. $z$ is an output of another neural network $f_{\theta}(x)$.

\begin{equation} \label{eq:attention1}
a = f_{\phi}(x)
\end{equation}

\begin{equation} \label{eq:attention2}
g = a \odot z
\end{equation}

...
\projectinfo{
  ...
  abstract.en={In this thesis, the stylistic generation of natural language in dialogue systems is explored. The goal is to provide insight into the current state of research in this area and find promising research directions to the future. Feature-based modifications and a model for weighted decoding were implemented. The realization of the model is presented in two versions. The first one is a LSTM-based baseline and the second one uses state-of-the-art pretrained models for text generation. The experiments showed that when combining models, trained on certain styles, by weighted decoding, it is possible to achieve the generation of stylistic text, but there is a problem with the context. Based on these findings, recommendations and possible future research areas are suggested.}, % abstrakt v anglickém jazyce / abstract in english
...
Hlavním cílem této práce bylo vytvoření dialogového systému, který je schopen generovat text v různých stylech a ovládat projevy každého stylu. Úvodní část práce se zabývá přístupy NLG a popisem problémů v dialogových systémech. Následující kapitola popisuje nejčastěji používané techniky při generování přirozeného jazyka. Začíná to vysvětlením základních technik společných pro mnoho přístupů strojového učení a poté pokračuje v popisu modelů specifických pro NLG a použitých v experimentech v této práci. 

Další kapitola obsahuje portfolio současných metod a modelů řešení problémů v dialogových systémech. Kromě technik používaných k vytváření modelů, jsou také popsány datasety a metody jejich evaluace. Konkrétněji se práce zabývá metodami lidského hodnocení. 

Po vysvětlení evaluačních technik a datasetů následuje kapitola, představující návrh vlastní architektury dialogového systému. Tato architektura používá vážené dekódování jazykového modelu a stylizovaných jazykových modelů a je implementována ve dvou variantách. První varianta využívá modely založené na LSTM a druhá varianta využívá předškolené modely založené na transformerech. V následující kapitole se nachází popis implementace této architektury a její evaluaci.
V další kapitole jsou diskutovány výsledky experimentů. }
